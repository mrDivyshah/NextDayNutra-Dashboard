...
function AgentDetailContent() {
  ...
  // Resolve agent name from the jiraId
  useEffect(() => {
    fetch(`/api/jira/customers?type=agent`)
      .then(r => r.json())
      .then((agents: { name: string; jiraId: string }[]) => {
        const match = agents.find(a => a.jiraId === agentId);
        if (match) setAgentName(match.name);
        else setAgentName(agentId); // fallback to ID display
      })
      .catch(() => setAgentName(agentId));
  }, [agentId]);

  // Load orders filtered by this agent
  useEffect(() => {
    if (!agentName) return;
    setIsLoading(true);
    fetch(`/api/jira/orders?view=agent&agentName=${encodeURIComponent(agentName)}`)
      .then(r => r.json())
      .then(d => {
      ...
      <div style={{ padding: "24px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: 14, marginBottom: 24 }}>
          {[
            ...
            { label: "Completed", value: completedOrders.length, color: "#7c3aed", bg: "#ede9fe" },
            { label: "Total Commission Paid", value: `$${allOrders.reduce((s,o) => s + (o.commissionPaid||0), 0).toLocaleString(undefined, {minimumFractionDigits:2})}`, color: "#ea580c", bg: "#fff7ed" },
          ].map(s => (
        ...
        <div style={{ background: "#fff", borderRadius: 16, border: "1px solid #e2e8f0", overflow: "hidden" }}>
          <div style={{ padding: "14px 20px 0", borderBottom: "1px solid #f1f5f9" }}>
            <div style={{ fontSize: 15, fontWeight: 800, color: "#0f172a", marginBottom: 12 }}>Orders</div>
            {/* Tab bar */}
          ...
          <div style={{ overflowX: "auto", padding: "0 20px 20px" }}>
            {isLoading ? (
              ...
              <table style={{ width: "100%", borderCollapse: "separate", borderSpacing: "0 6px", minWidth: 1000 }}>
                <thead>
                  <tr>
                    {["Order Health","Sales Order #","CM Key","Product","Customer","Status","Qty","EST Ship","Actions"].map(h => (
                  ...
                  {displayOrders.map(order => {
                    ...
                    return (
                      <React.Fragment key={order.id}>
                        <tr className="order-row" style={{ background: "#fff", cursor: "pointer", borderRadius: 12 }} onClick={() => toggleRow(order.id, orderKey)}>
                          ...
                          <td style={{ ...tdStyle(false, false), fontWeight: 600 }}>{order.customer}</td>
                          ...
                          <tr>
                            <td colSpan={9} style={{ padding: "0 0 8px" }}>
                              <div style={{ background: "#f8fafc", borderRadius: 12, margin: "0 2px", padding: 20, border: "1px solid #e2e8f0" }}>
                                ...
                                {/* Commission info */}
                                <div style={{ background: "#fff", borderRadius: 12, padding: 16, border: "1px solid #e2e8f0" }}>
                                  <div style={{ fontSize: 13, fontWeight: 700, color: "#0f172a", marginBottom: 12 }}>Commission</div>
                                  <div style={{ display: "flex", gap: 12 }}>
                                    {[
                                      { lbl: "Total Commission", val: `$${(order.commissionTotal||0).toLocaleString(undefined,{minimumFractionDigits:2})}`, color: "#0f172a" },
                                      { lbl: "Paid", val: `$${(order.commissionPaid||0).toLocaleString(undefined,{minimumFractionDigits:2})}`, color: "#16a34a" },
                                      { lbl: "Due", val: `$${(order.commissionDue||0).toLocaleString(undefined,{minimumFractionDigits:2})}`, color: "#f97316" },
                                      { lbl: "Balance Owed", val: `$${(order.commissionBalanceOwed||0).toLocaleString(undefined,{minimumFractionDigits:2})}`, color: "#dc2626" },
                                    ].map(item => (
                                      <div key={item.lbl} style={{ flex: 1, background: "#f8fafc", borderRadius: 8, padding: "10px 12px" }}>
                                        <div style={{ fontSize: 10, color: "#64748b", textTransform: "uppercase", fontWeight: 600, marginBottom: 4 }}>{item.lbl}</div>
