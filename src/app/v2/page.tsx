...
const MOCK_CUSTOMERS: Customer[] = [
  { id: 1, name: "Bigly", agent: "Rachel", users: [{ id: 101, name: "Bigly User", email: "user@bigly.com", role: "Customer" }] },
  { id: 2, name: "Wellnex Solutions", agent: "Rachel", users: [{ id: 201, name: "Caitlin Macnabb", email: "caitlin@wellnex.com", role: "Customer" }] },
  { id: 3, name: "Test Customer 2", agent: "Shubham", users: [{ id: 301, name: "Divy", email: "divy@test.com", role: "Customer" }] },
];
...
function V2Dashboard() {
  ...
  const [activeJiraCustomers, setActiveJiraCustomers] = useState<{name: string, agent: string}[]>([]);
  ...
  useEffect(() => {
    ...
    fetch("/api/wordpress/users?roles=agent,customer,customer-agent")
      .then(r => r.ok ? r.json() : Promise.reject(r.status))
      .then((data: any[]) => {
        if (Array.isArray(data) && data.length > 0) {
          setLiveCustomers(data.map(u => ({
            id: u.id,
          ...
          setLiveCustomers(MOCK_CUSTOMERS);
        }
      })
  ...
  const filteredUsersList = useMemo((): Customer[] => {
    if (viewMode === "agent") return activeJiraAgents.map((name, i) => ({ id: -(i + 1001), name, users: [], isAgent: true }));
    if (viewMode === "internal") return [];
    return activeJiraCustomers.map((item, i) => ({ id: -(i + 1), name: item.name, users: [], isAgent: false, agent: item.agent }));
  ...
  const resolveCustomerId = (name: string) => {
    ...
    const m = liveCustomers.find(c => c.name.trim().toLowerCase() === n);
  ...
  const toggleUserPanel = (id: number) =>
    setExpandedUserPanels(prev => prev.includes(id) ? prev.filter(r => r !== id) : [...prev, id]);
...
function AgentHierarchyView({ allOrders, allAgents, allAssignments, isLoading, renderOrdersTable }: { 
  ...
  allAgents: string[], 
  allAssignments: { name: string, agent: string }[],
  isLoading: boolean,
  ...
  const grouped = useMemo(() => {
    ...
    const targetAgents = agentFilter === "All Agents" ? allAgents : [agentFilter];

    targetAgents.forEach(agent => {
      if (agent !== "Unassigned") {
        map[agent] = {};
        // Pre-fill with assigned customers to ensure they show up even with 0 orders
      ...
      <div style={{ background: "#fff", padding: "12px 20px", borderRadius: 12, border: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap", boxShadow: "0 2px 4px rgba(0,0,0,0.02)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
          ...
          <select value={agentFilter} onChange={e => setAgentFilter(e.target.value)} style={{ ...selStyle, width: 180 }}>
            ...
            {allAgents.filter(a => a !== "Unassigned").sort().map(a => <option key={a} value={a}>{a}</option>)}
          </select>
        </div>
...
function CustomerOrdersView({ viewMode, filteredUsersList, selectedCustomerId, setSelectedCustomerId, selectedCustomer, isLoadingUsers, isLoadingJira, isLoadingOrders, allOrders, renderOrdersTable }: {
